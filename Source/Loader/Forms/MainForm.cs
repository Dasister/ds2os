...
namespace Loader
{
    public partial class MainForm : Form
    {
        ...
        private void OnImportServerConfig(object sender, EventArgs e)
        {
            using (OpenFileDialog Dialog = new OpenFileDialog())
            {
                Dialog.Filter = "Dark Souls II - Server Config|*.ds2osconfig|All Files|*.*";
        ...
        void PerformLaunch(ServerConfig Config)
        {
            ...
            byte[] NullTerminatorDataBlock = new byte[DataBlock.Length + 1];
            Array.Copy(DataBlock, NullTerminatorDataBlock, DataBlock.Length);
            ...
            if (!Result)
            {
                MessageBox.Show("Failed to launch data souls 2 executable.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // DS2 seems to be wrapped in some kind of launcher. When started as suspended the process is only 64kb and contains the stub loader.
            // So as a hacky work around we just wait until our working set has grown to a reasonable amount, assume everything has been loaded by then, and then patch.
            Process ManagedProcess = Process.GetProcessById((int)ProcessInfo.dwProcessId);
            while (!ManagedProcess.HasExited && ManagedProcess.PeakVirtualMemorySize64 < 100 * 1024 * 1024)
            {
                Thread.Sleep(1000);
            }

            // Dark souls 2 uses ASLR, so we need to calculate patch address relative to module base.
            IntPtr PatchAddress = (IntPtr)((ulong)ManagedProcess.MainModule.BaseAddress + (ulong)LoadConfig.ServerInfoAddress);            

            int BytesWritten;
//            uint oldProtection = 0;
//            bool Ret = WinAPI.VirtualProtectEx(ProcessInfo.hProcess, PatchAddress, (uint)NullTerminatorDataBlock.Length, (uint)MemoryProtectionMode.PAGE_EXECUTE_READWRITE, out oldProtection);
//            int Error = WinAPI.GetLastError();
            bool WriteSuccessful = WinAPI.WriteProcessMemory(ProcessInfo.hProcess, PatchAddress, NullTerminatorDataBlock, (uint)NullTerminatorDataBlock.Length, out BytesWritten);
            int Error = WinAPI.GetLastError();
//            Ret = WinAPI.VirtualProtectEx(ProcessInfo.hProcess, PatchAddress, (uint)NullTerminatorDataBlock.Length, oldProtection, out oldProtection);
//            Error = WinAPI.GetLastError();
        ...
        private void ClickGithubLink(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            {
                FileName = "https://github.com/tleonarduk/ds2os",
